...
const MailContextProvider = ({children}) => {
    ...
    const reducer = (state, action) => {
        switch(action.type){
            case "SET_MAIL_DATA" : {
                return {...state, mailData : action.mailData, filteredMail : action.mailData}
            }
            case "SET_SPAM" : {
                return {...state, spam : [...state.spam, action.mail], filteredMail : state.filteredMail.filter(({mId}) => mId !== action.mail.mId)}
            }

            case "RESTORE_SPAM" :{
                return {...state, spam : state.spam.filter(({mId}) => mId !== action.mail.mId), filteredMail : [...state.filteredMail, action.mail]}
            }

           
            case "SET_TRASH": {
                return {...state, trash : [...state.trash, action.mail], filteredMail : state.filteredMail.filter(({mId}) => mId !== action.mail.mId)}
            }

            case "RESTORE_TRASH" :{
                return {...state, trash : state.trash.filter(({mId}) => mId !== action.mail.mId), filteredMail : [...state.filteredMail, action.mail]}
            }
            ...
            case "MARK_AS_READ" : {
                ...
                return {...state, mailData : state.mailData.map((mail) => {
            ...
            case "TOGGLE_STAR_SPAM" : {
                return {...state, filteredMail : state.filteredMail.map((mail) => {
                    if(mail.mId === action.mailId){
                        return {...mail, isStarred : !mail.isStarred}
                    }
                    else{
            ...
            case "FILTER_MAIL" : {
                ...
                if(state.showStarred) {
                    newFilteredMail = newFilteredMail.filter(({isStarred}) => isStarred)
    ...
    return <MailContext.Provider value={{state, dispatch}}>
        {children}
